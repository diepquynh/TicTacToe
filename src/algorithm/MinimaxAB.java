...
/*
 * Minimax with alpha-beta pruning bot algorithm
 * original source: https://github.com/canberkakcali/gomoku-ai-minimax/blob/master/src/Minimax.java
 *
 * Bot difficulty will be adjusted by setting search depth
 */
...
public class MinimaxAB {
    ...
    // This function calculates the relative score of the bot player against the human.
    // (i.e. how likely is bot player to win the game before the human player)
    // This value will be used as the score in the Minimax algorithm.
    public static double evaluateBoardForBot(Board board, boolean playersTurn) {
        // Get board score of both players.
        double playerScore = getScore(board, true, playersTurn);
        double botScore = getScore(board, false, playersTurn);

        if (playerScore == 0) playerScore = 1.0;

        // Calculate relative score of bot against human
    ...
    public static int getScore(Board board, boolean forPlayer, boolean playersTurn) {
        // Calculate score for each of the 3 directions
        return evaluateHorizontal(board, forPlayer, playersTurn) +
                evaluateVertical(board, forPlayer, playersTurn) +
                evaluateDiagonal(board, forPlayer, playersTurn);
    }

    // This function is used to get the next intelligent move to make for the AI.
    public static int[] calculateNextMove(Board board, int depth) {
        int[] move = new int[2];
    ...
    private static Object[] minimaxSearchAB(int depth, Board board, boolean max, double alpha, double beta) {
        ...
        if (depth == 0) {
            Object[] x = {evaluateBoardForBot(board, !max), null, null};
            return x;
        }
        ...
        if (max) {
            ...
            for (int[] move : allPossibleMoves) {
                ...
                dummyBoard.setMarkAt(move[0], move[1], O);
                ...
                // (if the depth > 0) and searches for the minimum bot score in each of the sub trees.
            ...
            for (int[] move : allPossibleMoves) {
                ...
                // Play the move on that temporary board without drawing anything
                dummyBoard.setMarkAt(move[0], move[1], X);

                // Call the minimax function for the next depth, to look for a maximum score.
                // This function recursively generates new Minimax trees branching from this node
                // (if the depth > 0) and searches for the maximum bot score in each of the sub trees.
    ...
    public static int evaluateHorizontal(Board board, boolean forPlayer, boolean playersTurn) {
        int consecutive = 0;
        ...
        for (int i = 0; i < board.getBoardSize(); i++) {
            ...
            for (int j = 0; j < board.getBoardSize(); j++) {
                ...
                if (board.getMarkAt(i, j) == (forPlayer ? X : O)) {
                ...
                // Check if cell is empty
                    ...
                    if (consecutive > 0) {
                        ...
                        score += getConsecutiveSetScore(consecutive, blocks, forPlayer == playersTurn);
    ...
    public static int evaluateVertical(Board board, boolean forPlayer, boolean playersTurn) {
        int consecutive = 0;
        int blocks = 2;
        int score = 0;
    ...
    public static int evaluateDiagonal(Board board, boolean forPlayer, boolean playersTurn) {
        int consecutive = 0;
        int blocks = 2;
        int score = 0;
        // From bottom-left to top-right diagonally
        for (int k = 0; k <= 2 * (board.getBoardSize() - 1); k++) {
            int iStart = Math.max(0, k - board.getBoardSize() + 1);
            int iEnd = Math.min(board.getBoardSize() - 1, k);
        ...
        for (int k = 1 - board.getBoardSize(); k < board.getBoardSize(); k++) {
            int iStart = Math.max(0, k);
            int iEnd = Math.min(board.getBoardSize() + k - 1, board.getBoardSize() - 1);
            for (int i = iStart; i <= iEnd; ++i) {
                int j = i - k;
